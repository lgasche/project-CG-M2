...
TunnelStorage::TunnelStorage(const std::map<std::tuple<unsigned int, unsigned int>, Tunnel> map_lvl_, const FilePath &applicationPath) : tunnels{map_lvl_}
{
    ...
    textureDoor.loadTexture("../assets/textures/Door.png");
}
...
bool TunnelStorage::canMove(std::tuple<unsigned int, unsigned int> pos)
{
    ...
    {
        if (it->second.get_pos() == pos)
            return true;
    }
    return false;
}

void TunnelStorage::drawTunnels(int vertex, glm::mat4 globalMVMatrix, glm::mat4 projMatrix, tuple<unsigned int, unsigned int> out)
{
    ...
    for (auto it = tunnels.begin(); it != tunnels.end(); it++)
    {
        if (it->second.haveNorthWall())
            addMatrix(vertex, projMatrix, it->second.drawNorthWall(globalMVMatrix));
        if (it->second.haveEastWall())
            addMatrix(vertex, projMatrix, it->second.drawEastWall(globalMVMatrix));
        if (it->second.haveSouthWall())
            addMatrix(vertex, projMatrix, it->second.drawSouthWall(globalMVMatrix));
        if (it->second.haveWestWall())
            addMatrix(vertex, projMatrix, it->second.drawWestWall(globalMVMatrix));
    }
    ...
    // Door.
    textureDoor.bindingFirstTexture();
    glm::mat4 Matrix;
    auto it = tunnels.find(out);
    if (!it->second.haveNorthWall())
        Matrix = it->second.drawNorthWall(glm::translate(globalMVMatrix, glm::vec3(0.f, 0.f, -1.f)));
    else if (!it->second.haveEastWall())
        Matrix = it->second.drawEastWall(glm::translate(globalMVMatrix, glm::vec3(-1.f, 0.f, -1.f)));
    else if (!it->second.haveSouthWall())
        Matrix = it->second.drawSouthWall(glm::translate(globalMVMatrix, glm::vec3(-1.f, 0.f, 0.f)));
