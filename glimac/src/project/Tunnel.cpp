...
void Tunnel::drawWalls(int vertex, BasicProgram *squareProgam, glm::mat4 globalMVMatrix, glm::mat4 projMatrix)
{
    ...
    if (wall_west)
    {
        wallMVMatrix = glm::translate(globalMVMatrix, glm::vec3(get<0>(pos), 0.f, get<1>(pos))); // Translation
    ...
    if (wall_east)
    {
        wallMVMatrix = glm::translate(globalMVMatrix, glm::vec3(get<0>(pos) + 1.f, 0.f, get<1>(pos) + 1.f));         // Translation (décalage de 1 en X et en Y a cause de la rotation)
    ...
    if (wall_north)
    {
        wallMVMatrix = glm::translate(globalMVMatrix, glm::vec3(get<0>(pos) + 1.f, 0.f, get<1>(pos)));               // Translation (décalage de 1 en X a cause de la rotation)
    ...
    if (wall_south)
    {
        wallMVMatrix = glm::translate(globalMVMatrix, glm::vec3(get<0>(pos), 0.f, get<1>(pos) + 1.f));              // Translation (décalage de 1 en X a cause de la rotation)
        wallMVMatrix = glm::rotate(wallMVMatrix, glm::radians(90.f), glm::vec3(0.f, 1.f, 0.f)); // Translation * Rotation
        glUniformMatrix4fv(squareProgam->uMVPMatrix, 1, GL_FALSE, glm::value_ptr(projMatrix * wallMVMatrix));
        glUniformMatrix4fv(squareProgam->uMVMatrix, 1, GL_FALSE, glm::value_ptr(wallMVMatrix));
        glUniformMatrix4fv(squareProgam->uNormalMatrix, 1, GL_FALSE, glm::value_ptr(glm::transpose(glm::inverse(wallMVMatrix))));
